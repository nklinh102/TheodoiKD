...
import { Download, Save, Loader2, Sparkles, AlertCircle, Camera } from "lucide-react";
...
export function ManulifeProBonusTable() {
    ...
    const stats = {
        ...
        bonusRecipients: data.filter(a => (a.bonus_amount || 0) > 0).length,
        bonusRate: data.length > 0 ? (data.filter(a => (a.bonus_amount || 0) > 0).length / data.length) * 100 : 0,
        rankCounts: {
            platinum: data.filter(a => a.pro_type.includes("Bạch Kim")).length,
            gold: data.filter(a => a.pro_type.includes("Vàng")).length,
            silver: data.filter(a => a.pro_type.includes("Bạc") && !a.pro_type.includes("Bạch Kim")).length,
        }
    };
        ...
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex flex-col justify-center border-l-4 border-l-emerald-500">
                    <span className="text-emerald-600 text-[10px] font-black uppercase tracking-widest mb-1">Tổng FYP Tháng T (nghìn VNĐ)</span>
                    <div className="flex items-baseline gap-1">
                        <span className="text-2xl font-black text-slate-900 leading-none">{formatCurrency(stats.totalFyp)}</span>
                        <span className="text-slate-400 text-xs font-bold whitespace-nowrap">nghìn VNĐ</span>
                    </div>
                </div>
                <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex flex-col justify-center border-l-4 border-l-yellow-500">
                    <span className="text-yellow-600 text-[10px] font-black uppercase tracking-widest mb-1.5">Hiệu quả nhận thưởng</span>
                    <div className="flex items-center justify-between">
                        <div className="flex items-baseline gap-1">
                            <span className="text-2xl font-black text-slate-900 leading-none">{stats.bonusRecipients}</span>
                            <span className="text-slate-400 text-[10px] font-bold uppercase">Người</span>
                        </div>
                        <div className="flex items-center gap-1.5 bg-yellow-50 px-2 py-1 rounded-lg border border-yellow-100">
                            <Sparkles className="w-3 h-3 text-yellow-600" />
                            <span className="text-sm font-black text-yellow-700">{stats.bonusRate.toFixed(1)}%</span>
                        </div>
                    </div>
                </div>
            ...
            <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                <div className="flex items-center gap-2">
                    {/* Placeholder or Empty if nothing else needed on left */}
                </div>
                <Button
            ...
            <div ref={captureRef} className="bg-white border border-slate-200 rounded-[32px] overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.05)] relative p-8">
                ...
                {/* Standardized Header */}
                <div className="bg-white p-3 px-5 border-b border-slate-100 relative overflow-hidden flex justify-between items-center mb-4 rounded-t-3xl">
                    ...
                    <Table className="border-separate border-spacing-y-1">
                        <TableHeader>
                            <TableRow className="bg-emerald-600 hover:bg-emerald-600 border-none overflow-hidden">
                                ...
                                <TableHead className="text-right text-white font-black uppercase text-[10px] tracking-widest border-none">FYP Tháng T<br />(nghìn VNĐ)</TableHead>
                                <TableHead className="text-right text-white font-black uppercase text-[10px] tracking-widest border-none">Mức Thưởng<br />(nghìn VNĐ)</TableHead>
            ...
            <div className="bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-sm">
                <div className="bg-slate-50 px-8 py-6 border-b border-slate-200">
                    <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight flex items-center gap-3">
                        ...
                        Bảng Quy Định Mức Thưởng (đơn vị: nghìn VNĐ)
                    </h3>
                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
                        ...
                        <div className="space-y-4 text-sm">
                            ...
                            <div className="space-y-1">
                                <div className="flex justify-between p-2.5 bg-emerald-50/50 rounded-xl"><span>≥ 120 triệu đồng (*)</span> <span className="font-black text-emerald-700">20.000*</span></div>
                                <div className="flex justify-between p-2.5"><span>≥ 100 triệu đồng</span> <span className="font-black text-emerald-700">16.000</span></div>
                                <div className="flex justify-between p-2.5 bg-emerald-50/50 rounded-xl"><span>≥ 80 triệu đồng</span> <span className="font-black text-emerald-700">12.000</span></div>
                                <div className="flex justify-between p-2.5"><span>≥ 60 triệu đồng</span> <span className="font-black text-emerald-700">10.000</span></div>
                                <div className="flex justify-between p-2.5 bg-emerald-50/50 rounded-xl"><span>≥ 45 triệu đồng</span> <span className="font-black text-emerald-700">8.000</span></div>
                                <div className="flex justify-between p-2.5"><span>≥ 30 triệu đồng</span> <span className="font-black text-emerald-700">4.000</span></div>
                                <div className="flex justify-between p-2.5 bg-emerald-50/50 rounded-xl"><span>≥ 15 triệu đồng</span> <span className="font-black text-emerald-700">2.000</span></div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-4 text-sm">
                            ...
                            <div className="space-y-1">
                                <div className="flex justify-between p-2.5 bg-yellow-50/30 rounded-xl"><span>≥ 60 triệu đồng</span> <span className="font-black text-yellow-700">7.000</span></div>
                                <div className="flex justify-between p-2.5"><span>≥ 45 triệu đồng</span> <span className="font-black text-yellow-700">5.000</span></div>
                                <div className="flex justify-between p-2.5 bg-yellow-50/30 rounded-xl"><span>≥ 30 triệu đồng</span> <span className="font-black text-yellow-700">3.000</span></div>
                                <div className="flex justify-between p-2.5"><span>≥ 15 triệu đồng</span> <span className="font-black text-yellow-700">1.500</span></div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-4 text-sm">
                            ...
                            <div className="space-y-1">
                                <div className="flex justify-between p-2.5 bg-slate-50 rounded-xl"><span>≥ 30 triệu đồng</span> <span className="font-black text-slate-700">3.000</span></div>
                                <div className="flex justify-between p-2.5 bg-slate-50 rounded-xl"><span>≥ 15 triệu đồng</span> <span className="font-black text-slate-700">1.500</span></div>
                            </div>
                        </div>
                    </div>
                </div>
